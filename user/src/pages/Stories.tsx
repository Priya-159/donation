    ...
    <div className={`min-h-screen pt-24 pb-16 ${dark ? 'bg-slate-900' : 'bg-white'}`}>
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex flex-wrap gap-2 mb-10 justify-center">
          <button onClick={() => setSelected(null)} className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all ${!selected ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/25' : dark ? 'bg-slate-800 text-gray-400 hover:text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            All Stories
          </button>
          {cats.map(c => (
            <button key={c} onClick={() => setSelected(c)} className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all ${selected === c ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/25' : dark ? 'bg-slate-800 text-gray-400 hover:text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
              {c}
            </button>
          ))}
        </div>
        ...
        <div className="mt-16 text-center">
          <div className={`rounded-3xl p-8 sm:p-12 bg-gradient-to-br from-primary-500 to-accent-600 relative overflow-hidden`}>
            ...
            <div className="relative z-10">
              ...
              <Link to="/donate" className="inline-flex items-center gap-2 px-8 py-4 bg-white text-primary-700 rounded-2xl text-lg font-semibold hover:bg-gray-50 transition-all shadow-xl hover:-translate-y-1">
