...
import { Heart, Target, Eye, BookOpen, Users, TreePine, Utensils, HandHeart, Award, Globe, ChevronRight } from 'lucide-react';
      ...
      <section className={`py-16 ${dark ? 'bg-slate-800/50' : 'bg-gray-50'}`}>
        <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className={`rounded-3xl p-8 transition-all hover:-translate-y-1 hover:shadow-xl ${dark ? 'bg-slate-800' : 'bg-white shadow-lg'}`}>
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center mb-4 shadow-lg">
          ...
          <div className={`rounded-3xl p-8 transition-all hover:-translate-y-1 hover:shadow-xl ${dark ? 'bg-slate-800' : 'bg-white shadow-lg'}`}>
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-accent-400 to-accent-600 flex items-center justify-center mb-4 shadow-lg">
          ...
          <div>
            ...
            <div className="mt-6 flex gap-4">
              <div className={`rounded-xl p-4 flex-1 ${dark ? 'bg-primary-900/20' : 'bg-primary-50'}`}>
                <div className={`text-2xl font-bold ${dark ? 'text-primary-400' : 'text-primary-600'}`}>6+</div>
              ...
              <div className={`rounded-xl p-4 flex-1 ${dark ? 'bg-accent-900/20' : 'bg-accent-50'}`}>
                <div className={`text-2xl font-bold ${dark ? 'text-accent-400' : 'text-accent-600'}`}>50+</div>
          ...
          <div className="relative">
            ...
            {timeline.map((item, i) => (
              <div key={i} className={`relative flex items-start mb-8 ${i % 2 === 0 ? 'sm:flex-row' : 'sm:flex-row-reverse'}`}>
                ...
                <div className={`absolute left-4 sm:left-1/2 -translate-x-1/2 w-8 h-8 rounded-full flex items-center justify-center z-10 bg-primary-500 shadow-md shadow-primary-500/30`}>
      ...
      <section className={`py-16 ${dark ? 'bg-slate-800/50' : 'bg-gray-50'}`}>
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: Heart, title: 'Compassion', desc: 'Every action driven by genuine care for communities' },
              { icon: Globe, title: 'Transparency', desc: '100% visibility into how donations are utilized' },
              { icon: Award, title: 'Excellence', desc: 'Striving for the highest impact in everything we do' },
              { icon: BookOpen, title: 'Education', desc: 'Empowering through knowledge and awareness' },
            ].map((v, i) => (
              <div key={i} className={`text-center p-6 rounded-2xl transition-all hover:-translate-y-1 hover:shadow-lg ${dark ? 'bg-slate-800' : 'bg-white shadow-sm border border-gray-100'}`}>
                <v.icon className={`w-8 h-8 mx-auto mb-3 ${dark ? 'text-primary-400' : 'text-primary-500'}`} />
                <h3 className={`font-bold mb-2 ${dark ? 'text-white' : 'text-gray-900'}`}>{v.title}</h3>
                <p className={`text-sm ${dark ? 'text-gray-400' : 'text-gray-500'}`}>{v.desc}</p>
              </div>
            ))}
          </div>
        </div>
