...
export default function Volunteer() {
  ...
  useEffect(() => {
    ...
    checkStatusChanges();
    const interval = setInterval(checkStatusChanges, 60000); // Check every minute
            ...
            <div className="space-y-4">
              {roles.map(role => (
                <button key={role.value} onClick={() => update('role', role.value)}
                  ...
                  {form.role === role.value && <CheckCircle className="w-5 h-5 text-primary-500 ml-auto flex-shrink-0" />}
                </button>
              ))}
            </div>
          ...
          <div className={`rounded-3xl p-6 sm:p-8 ${dark ? 'bg-slate-800' : 'bg-white shadow-xl border border-gray-100'}`}>
              ...
              <div className={`mb-6 p-4 rounded-2xl border-2 ${dark ? 'border-slate-600 bg-slate-700/30' : 'border-primary-200 bg-primary-50/50'}`}>
                ...
                {pastApplications.map((app: any) => (
                  <div key={app.id} className="flex justify-between text-xs mt-1">
                    ...
                    <span className={`px-2 py-0.5 rounded-full font-medium ${app.status === 'Approved' ? 'bg-green-100 text-green-700' : app.status === 'Rejected' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{app.status}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <form onSubmit={async (e) => {
              ...
              try {
                ...
                setErrorMsg(err.message || 'Submission failed. Please make sure you are logged in.');
              } finally {
                setLoading(false);
              }
            }} className="space-y-4">
              {[
                ...
                <div key={f.key} className="relative">
                  ...
                  <input type={f.type} placeholder={f.label} value={form[f.key as keyof typeof form]} onChange={e => update(f.key, e.target.value)}
                    className={`w-full pl-10 pr-4 py-3 rounded-xl border-2 text-sm ${dark ? 'bg-slate-700 border-slate-600 text-white placeholder:text-gray-500 focus:border-primary-500' : 'bg-gray-50 border-gray-200 focus:border-primary-500 focus:bg-white'} outline-none transition-colors`} />
                </div>
              ))}
              <textarea placeholder="Why do you want to volunteer? (Optional)" value={form.message} onChange={e => update('message', e.target.value)} rows={3}
                className={`w-full px-4 py-3 rounded-xl border-2 text-sm resize-none ${dark ? 'bg-slate-700 border-slate-600 text-white placeholder:text-gray-500 focus:border-primary-500' : 'bg-gray-50 border-gray-200 focus:border-primary-500 focus:bg-white'} outline-none transition-colors`} />
              <button type="submit" disabled={loading} className="w-full flex justify-center py-3.5 bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-xl font-semibold shadow-lg shadow-primary-500/25 hover:shadow-xl hover:-translate-y-0.5 transition-all disabled:opacity-60">
